...
ffi.cdef(
    ...
    // Install action
    int helmpy_install(helmpy_handle handle, const char *release_name, const char *chart_path, const char *values_json, char **result_json);

    // Upgrade action
    int helmpy_upgrade(helmpy_handle handle, const char *release_name, const char *chart_path, const char *values_json, char **result_json);

    // Uninstall action
    int helmpy_uninstall(helmpy_handle handle, const char *release_name, char **result_json);
    ...
    with _library_lock:
        ...
        if lib_path is None:
            raise HelmLibraryNotFound(
                "Could not find helmpy shared library. "
                "Please ensure helmpy is properly installed."
            )
        ...
        try:
            ...
            raise HelmLibraryNotFound(
                f"Failed to load helmpy library from {lib_path}: {e}"
            ) from e
